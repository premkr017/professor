...
<?php $currentType = old('type', $transaction->type); ?>

@section('content')
    <div class="card" style="max-width:760px">
        <div class="card-header">
            <div class="card-title">Edit Transaction #{{ $transaction->id }}</div>
        </div>
        ...
        <form method="POST" action="{{ route('transactions.update', $transaction) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="payment_method" id="payment_method" class="form-control">
                        ...
                        @foreach($paymentMethods as $value => $label)
                            <option value="{{ $value }}" {{ old('payment_method', $transaction->payment_method ?? 'cash') === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="payee" id="payee" class="form-control" value="{{ old('payee', $transaction->payee) }}" placeholder="e.g. Big Bazaar, Salary, Ravi">
                @error('payee')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="form-control" rows="3" placeholder="Monthly grocery">{{ old('description', $transaction->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                @if($transaction->attachment)
                    <div style="margin-bottom:10px">
                        <a href="{{ asset('storage/' . $transaction->attachment) }}" target="_blank" style="text-decoration:none">📎 View current receipt</a>
                    </div>
                @endif
                <input type="file" name="attachment" id="attachment" class="form-control" accept=".jpg,.jpeg,.png,.pdf,.webp">
                <div style="font-size:12px;color:#94a3b8;margin-top:6px">JPG, PNG, PDF or WEBP up to 5MB. Leave empty to keep current.</div>
                @error('attachment')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
